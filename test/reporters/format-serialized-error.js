...
test('print multiple values', t => {
	const err = {
		...
		values: [
			{
				label: 'Actual:',
				formatted: kathryn.format([1, 2, 3])
			},
			{
				label: 'Expected:',
				formatted: kathryn.format({a: 1, b: 2, c: 3})
			}
		]
	};
...
test('print multiple statements', t => {
	const err = {
		statements: [
			['actual.a[0]', kathryn.format(1)],
			['actual.a', kathryn.format([1])],
			['actual', kathryn.format({a: [1]})]
		],
		values: []
	};

	t.is(formatSerializedError(err).formatted, [
		`actual.a[0]\n${chalk.grey('=>')} ${kathryn.format(1)}`,
		`actual.a\n${chalk.grey('=>')} ${kathryn.format([1])}`,
		`actual\n${chalk.grey('=>')} ${kathryn.format({a: [1]})}`
	].join('\n\n'));
	t.end();
});
