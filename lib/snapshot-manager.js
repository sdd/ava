...
const kathrynOptions = require('./kathryn-options').plain;
...
function formatEntry(label, descriptor) {
	...
	const codeBlock = indentString(kathryn.formatDescriptor(descriptor, kathrynOptions), 4);
...
class Manager {
	...
	compare(options) {
		...
		const actual = kathryn.deserialize(snapshotBuffer);

		const expected = kathryn.describe(options.expected);
	...
	record(hash, options) {
		const descriptor = kathryn.describe(options.expected);

		this.hasChanges = true;
		const snapshot = kathryn.serialize(descriptor);
